...
public class TweakTransformer implements IClassTransformer, Opcodes {
    ...
    public static void load() {
        ...
        tweaks = CodeChickenCoreModContainer.config
                .getTag("tweaks")
                .setComment("Various tweaks that can be applied to game mechanics.")
                .useBraces();
        ...
        if (tweaks.getTag("environmentallyFriendlyCreepers")
            ...
            transformer.add(new MethodReplacer(
                    new ObfMapping("net/minecraft/entity/monster/EntityCreeper", "func_146077_cc", "()V"),
                    blocks.get("d_environmentallyFriendlyCreepers"),
                    blocks.get("environmentallyFriendlyCreepers")));
        }

        if (!tweaks.getTag("softLeafReplace")
                .setComment("If set to false, leaves will only replace air when growing")
                .getBooleanValue(false)) {
            transformer.add(new MethodWriter(
                    ACC_PUBLIC,
                    new ObfMapping(
                            "net/minecraft/block/Block",
                            "canBeReplacedByLeaves",
                            "(Lnet/minecraft/world/IBlockAccess;III)Z"),
                    blocks.get("softLeafReplace")));
        }
        ...
        if (tweaks.getTag("doFireTickOut")
            ...
            transformer.add(
                    new MethodTransformer(new ObfMapping(
