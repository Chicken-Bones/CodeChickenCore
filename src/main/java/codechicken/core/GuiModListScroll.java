...
public class GuiModListScroll {
    ...
    public static void draw(GuiModList gui, int mouseX, int mouseY) {
        ...
        if (!scrollMods.contains(selectedMod) || selectedMod.getMetadata().autogenerated) return;
    ...
    private static int calcDescY(GuiModList gui, ModContainer mod) {
        ...
        int y = 35;
        if (!!meta.logoFile.isEmpty()
                && ReflectionManager.getField(GuiModList.class, ResourceLocation.class, gui, "cachedLogo") != null)
            y += 65;
        y += 12; // title
        y += 40; // necessary lines
        if (!meta.credits.isEmpty()) y += 10;
        if (!meta.childMods.isEmpty()) y += 10;
        return y;
    }
}
